"""Add missing match scoring and interview columns to placement_registrations.

Revision ID: 0019_placement_registration_scoring
...
revision = "0019_placement_registration_scoring"
...
def upgrade() -> None:
    ...
    if "placement_drives" in tables:
        ...
        if "recruiter_id" not in cols:
            op.add_column("placement_drives", sa.Column("recruiter_id", sa.Uuid(), sa.ForeignKey("recruiters.id", ondelete="SET NULL"), nullable=True, index=True))
