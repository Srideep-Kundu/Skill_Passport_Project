...
def upgrade() -> None:
    ...
    status = sa.Enum(
        "queued", "running", "completed", "partial", "failed", name="discoveryrunstatus"
    )
    if op.get_bind().dialect.name == "postgresql":
        op.execute(
            "DO $$ BEGIN CREATE TYPE discoveryrunstatus AS ENUM ('queued', 'running', 'completed', 'partial', 'failed'); EXCEPTION WHEN duplicate_object THEN NULL; END $$"
        )
    ...
    if "job_discoveries" not in tables:
        op.create_table(
            "job_discoveries",
            ...
            sa.Column(
                "freshness_days", sa.Integer(), nullable=False, server_default="30"
            ),
            sa.Column(
                "minimum_match_score",
                sa.Numeric(5, 4),
            ...
            sa.Column(
                "cadence_hours", sa.Integer(), nullable=False, server_default="24"
            ),
        ...
        op.create_index(
            "ix_job_discoveries_student_id", "job_discoveries", ["student_id"]
        )
        op.create_index("ix_job_discoveries_enabled", "job_discoveries", ["enabled"])
        op.create_index(
            "ix_job_discoveries_next_run_at", "job_discoveries", ["next_run_at"]
        )
    if "job_discovery_runs" not in tables:
        op.create_table(
            "job_discovery_runs",
            sa.Column("id", sa.Uuid(), primary_key=True),
            sa.Column(
                "discovery_id",
                sa.Uuid(),
                sa.ForeignKey("job_discoveries.id", ondelete="CASCADE"),
                nullable=False,
            ),
            sa.Column("status", status, nullable=False),
            sa.Column("providers_requested", sa.JSON(), nullable=False),
            sa.Column("provider_results", sa.JSON(), nullable=False),
        ...
        for name, columns in (
            ("ix_job_discovery_runs_discovery_id", ["discovery_id"]),
            ("ix_job_discovery_runs_status", ["status"]),
        ):
            op.create_index(name, "job_discovery_runs", columns)
    if "discovery_recommendations" not in tables:
        op.create_table(
            ...
            sa.Column(
                "external_job_id",
                sa.Uuid(),
                sa.ForeignKey("external_jobs.id"),
                nullable=False,
            ),
            sa.Column("match_fingerprint", sa.String(64), nullable=False),
            sa.Column(
                "first_recommended_at", sa.DateTime(timezone=True), nullable=False
            ),
            sa.Column(
                "last_recommended_at", sa.DateTime(timezone=True), nullable=False
            ),
            sa.UniqueConstraint(
                "discovery_id",
                "external_job_id",
                name="uq_discovery_recommendation_job",
            ),
        )
        op.create_index(
            "ix_discovery_recommendations_discovery_id",
            "discovery_recommendations",
            ["discovery_id"],
        )
        op.create_index(
            "ix_discovery_recommendations_external_job_id",
            "discovery_recommendations",
            ["external_job_id"],
        )
