...
# =============================================================================
# PROFESSIONAL SOCIETIES & INDUSTRY EXPERTS SCHEMAS
# =============================================================================

class ProfessionalSocietyResponse(APIModel):
    id: UUID
    name: str
    short_name: str
    description: str
    ...
    expert_speakers: list[dict[str, Any]]
    previous_collaborations: list[str]
    contact_email: str
    proposal_guidelines: str
    sponsorship_available: bool
    created_at: datetime


class ProfessionalSocietyListResponse(APIModel):
    total: int
    items: list[ProfessionalSocietyResponse]
    all_domains: list[str]


class IndustryExpertResponse(APIModel):
    id: UUID
    name: str
    organization: str
    title: str
    ...
    email: str
    rating: float
    past_sessions_count: int
    avatar_url: str | None = None


class IndustryExpertListResponse(APIModel):
    total: int
    items: list[IndustryExpertResponse]
    all_expertise: list[str]


# =============================================================================
# FUNDING EXPLORER & RECOMMENDATION SCHEMAS
# =============================================================================

class FundingOpportunityResponse(APIModel):
    id: UUID
    title: str
...
class FundingOpportunityListResponse(APIModel):
    total: int
...
class FundingRecommendationResponse(APIModel):
    opportunity: FundingOpportunityResponse
    match_score: float
    why_recommended: str
    relevant_department: str
    matched_student_gap: str | None = None


# =============================================================================
# PROPOSAL BUILDER & LIFECYCLE SCHEMAS
# =============================================================================

class FacultyProposalCreate(APIModel):
    society_id: UUID | None = None
    funding_id: UUID | None = None
    title: str = Field(min_length=3, max_length=255)
    objective: str = Field(min_length=10)
    event_type: str = Field(default="workshop", max_length=64)
    target_audience: str = Field(default="3rd & 4th Year Engineering Students", max_length=200)
    expected_participants: int = Field(default=60, ge=5, le=5000)
    required_funding: str = Field(default="₹45,000", max_length=100)
    funding_amount_numeric: float = Field(default=45000.0, ge=0)
    duration_days: int = Field(default=2, ge=1, le=30)
    proposed_dates: str = Field(default="", max_length=200)
    infrastructure_needed: list[str] = Field(default_factory=list)
    expected_outcomes: list[str] = Field(default_factory=list)
    budget_breakdown: dict[str, Any] = Field(default_factory=dict)


class FacultyProposalStatusUpdate(APIModel):
    status: str = Field(pattern="^(draft|submitted|under_review|accepted|rejected|active)$")
    reviewer_feedback: str | None = None
    actor_name: str = "Faculty Lead"


class ProposalEventResponse(APIModel):
    id: UUID
    proposal_id: UUID
    status: str
    note: str
...
class FacultyProposalResponse(APIModel):
    id: UUID
    faculty_id: UUID
    faculty_name: str | None = None
    society_id: UUID | None = None
    ...
    funding_id: UUID | None = None
    funding_title: str | None = None
    title: str
    objective: str
    event_type: str
    target_audience: str
    expected_participants: int
    required_funding: str
    funding_amount_numeric: float
    duration_days: int
    proposed_dates: str
...
class FacultyProposalListResponse(APIModel):
    total: int
    items: list[FacultyProposalResponse]


# =============================================================================
# TRAINING & WORKSHOP PLANNER SCHEMAS
# =============================================================================

class TrainingRecommendationItem(APIModel):
    program_title: str
    program_type: str
    target_skill: str
    ...
    recommended_trainer_name: str
    recommended_trainer_org: str
    recommended_trainer_id: UUID | None = None
    infrastructure_needed: list[str]
    estimated_cost: str
    suggested_collaborators: list[str]


class TrainingRecommendationListResponse(APIModel):
    recommendations: list[TrainingRecommendationItem]
    department: str
    overall_cohort_gap_skills: list[str]


class TrainingProgramCreate(APIModel):
    title: str = Field(min_length=3, max_length=255)
    description: str = Field(default="")
    program_type: str = Field(default="skill_gap_workshop", max_length=64)
    target_department: str = Field(default="Computer Science & Engineering", max_length=120)
    target_year: str = Field(default="3rd Year", max_length=64)
    target_skills: list[str] = Field(default_factory=list)
    expected_participants: int = Field(default=80, ge=5, le=5000)
    trainer_type: str = Field(default="external_expert", max_length=64)
    trainer_name: str = Field(default="", max_length=200)
    trainer_id: UUID | None = None
    start_date: datetime | None = None
    end_date: datetime | None = None
    budget_total: float = Field(default=0.0, ge=0)
    budget_breakdown: dict[str, Any] = Field(default_factory=dict)
    funding_gap: float = Field(default=0.0, ge=0)
    infrastructure_required: list[dict[str, Any]] = Field(default_factory=list)
    infrastructure_resolution: str | None = None
    status: str = Field(default="scheduled", max_length=32)


class MarketingKitResponse(APIModel):
    training_id: UUID
...
class TrainingOutcomeRecordRequest(APIModel):
    registered_count: int = Field(ge=0)
    attended_count: int = Field(ge=0)
    completed_count: int = Field(ge=0)
    feedback_rating: float = Field(default=4.6, ge=1.0, le=5.0)
    pre_workshop_readiness: float = Field(ge=0.0, le=100.0)
    post_workshop_readiness: float = Field(ge=0.0, le=100.0)
    skills_certified: list[str] = Field(default_factory=list)
    participant_student_ids: list[UUID] = Field(default_factory=list)
    notes: str | None = None


class TrainingOutcomeMetricResponse(APIModel):
    skill_name: str
    before_readiness: float
    after_readiness: float
...
class TrainingProgramResponse(APIModel):
    ...
    budget_breakdown: dict[str, Any]
    funding_gap: float
    infrastructure_required: list[dict[str, Any]]
    infrastructure_resolution: str | None = None
    marketing_kit: dict[str, Any] = Field(default_factory=dict)
    registered_count: int
    attended_count: int
    completed_count: int
    feedback_rating: float
    pre_workshop_readiness: float
    post_workshop_readiness: float
