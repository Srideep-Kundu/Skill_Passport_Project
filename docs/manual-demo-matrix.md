...
| Faculty Collaboration & Funding Hub | Log in as faculty, filter society/expert/collaborator/funding records, save one, inspect “Why Recommended?”, save and submit a proposal, move it through review as its owning recruiter, then accept it. | Search and amount/deadline/expertise/type filters are stable; saves are faculty-scoped; explanations cite faculty expertise, institution student-gap plans, and priorities using `faculty-hub-v1`; acceptance creates one active collaboration workspace linked to the proposal. |
| Faculty Training & Workshop Planner | Log in as `faculty.demo@example.com`, inspect a gap-driven recommendation, create a program, compare infrastructure and budget gaps, use Find Funding, and record attendance plus pre/post outcomes. | Recommendations cite persisted cohort-gap inputs; readiness reflects event lead time and pending tasks; marketing covers email, WhatsApp, LinkedIn, poster, and registration copy; attendance never auto-verifies a Skill Passport skill. |
