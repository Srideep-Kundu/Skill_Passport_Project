...
| Explicit assessments | Complete one soft-skill or aptitude assessment. | Deterministic per-competency scores create one assessment evidence record and provenance-linked Passport skills without LLM scoring. |
| Learning and internship outcomes | Complete one recruiter-verified learning program and one mentor-verified internship lifecycle. | Each accepted completion creates idempotent evidence and refreshes the Passport/readiness views. |
| Unified applications and placement pipeline | Open **My Applications**, then advance a recruiter-owned placement candidate through an allowed stage. | External, internship, and placement applications share a truthful timeline; recruiter ownership and transition rules remain enforced. |
| Public Passport | Create a scoped share, open its public URL/QR/PDF, then revoke it. | Only allowlisted fields appear; revoked tokens stop resolving without exposing private evidence. |
| Institution truthfulness and imports | Open an empty institution, then preview a governed import and inspect demand/supply. | Empty analytics remain empty; imported records are tenant-scoped; recommendations appear only when persisted demand exceeds qualified supply. |
