...
Set the following through platform-managed configuration, never in Git: `DATABASE_URL`, `REDIS_URL`, `JWT_SECRET`, extraction-provider keys (`GEMINI_API_KEY`, `GROQ_API_KEY`, and optional `COHERE_API_KEY`/`OPENROUTER_API_KEY`), `GITHUB_TOKEN`, `GOOGLE_CLIENT_ID`, the approved provider source identifiers, `INSTITUTION_REGISTRATION_ALLOWLIST`, and production `CORS_ORIGINS`. `JWT_SECRET_KEY` is accepted only for compatibility. Set `APP_ENV=production`; strong unique JWT keys, PostgreSQL, Key Value, an exact HTTPS CORS origin, and a non-empty Google audience when Google sign-in is exposed are mandatory. The complete extraction chain is configured as `cohere` with `groq,openrouter,gemini,local` fallbacks only when every listed provider has a valid server-side key. Gemini embeddings remain `gemini-embedding-001` at 768 dimensions. Semantic scoring must remain disabled until that compatible backfill succeeds.
