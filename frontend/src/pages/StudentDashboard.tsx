...
export function StudentDashboard({ token }: { token: string }) {
  ...
  const load = useCallback(async () => {
    setError(null);
    try { const [nextPassport, nextMatches, nextConsent] = await Promise.all([api.passport(token), api.studentMatches(token), api.recruiterEvidenceConsent(token)]); setPassport(nextPassport); setMatches(nextMatches); setRecruiterEvidenceConsent(nextConsent.recruiter_evidence_consent); }
