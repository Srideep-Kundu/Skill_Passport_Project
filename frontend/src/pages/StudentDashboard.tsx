...
export function StudentDashboard({ token }: { token: string }) {
  ...
  const load = useCallback(async () => {
    setError(null);
    try { const [nextPassport, nextMatches] = await Promise.all([api.passport(token), api.studentMatches(token)]); setPassport(nextPassport); setMatches(nextMatches); }
