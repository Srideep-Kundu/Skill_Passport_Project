...
export function ExternalJobs({ token }: { token: string }) {
  ...
  const load = useCallback(async () => {
    try {
      setError(null);
      const [jobPage, matchPage, applicationPage] = await Promise.all([api.externalJobs(token), api.externalJobMatches(token), api.applications(token)]);
      setJobs(jobPage.items); setRecommended(matchPage.items); setApplications(applicationPage.items);
  ...
  async function beginApplication(job: ExternalJobMatch) {
    ...
    try {
      ...
      setApplications((current) => [created, ...current]); setSelectedApplication(created);
