...
export function TrainingPlannerHub({ token, onNavigateToFundingHub }: TrainingPlannerHubProps) {
  ...
  const [wizardForm, setWizardForm] = useState<TrainingProgramCreateInput>({
    ...
    start_date: new Date(Date.now() + 41 * 86400000).toISOString().split("T")[0],
  ...
  async function loadData() {
    try {
      ...
      if (trRes.status === "fulfilled") {
        ...
        if (trRes.value.length > 0 && !selectedTraining) {
          setSelectedTraining(trRes.value[0]);
        }
      }
    } catch (err) {
  ...
  async function handleRecordOutcomesSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.success("Pre/Post training outcomes recorded and verified passport evidence issued to students!");
    } catch (err) {
  ...
  const eventDateMs = wizardForm.start_date ? new Date(wizardForm.start_date).getTime() : Date.now() + 41 * 86400000;
  const prepDays = Math.max(0, Math.round((eventDateMs - Date.now()) / (1000 * 60 * 60 * 24)));
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-[#475569] mt-1 font-mono">
              The Closed Loop: <span className="text-[#B08D57]">Skill Gap Detected</span> → <span className="text-[#B08D57]">Training Suggested</span> → <span className="text-[#B08D57]">Workshop Conducted</span> → <span className="text-[#4F6F5A]">Outcomes Measured & Passports Updated</span>.
            </p>
          </div>
            ...
            <div className="space-y-4 font-mono text-xs">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {[
                    "Address student skill gap",
                    "Faculty Development Program (FDP)",
                    "Industry exposure & Live Project",
          ...
          {wizardStep === 3 && (
            <div className="space-y-4 font-mono text-xs">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                {["Internal Faculty", "External Expert", "Industry Professional", "Professional Society", "Freelance Trainer"].map((tr) => (
                  <button
                    key={tr}
                    type="button"
              ...
              <div className="p-3 bg-[#FAF7F2] border border-[#B08D57]/30 rounded-sm space-y-1">
                ...
                <p className="text-[#475569]">
                  Dr. Arvind Swaminathan (Google Cloud & IEEE Senior Member) matches this curriculum with a 4.9★ rating across 32 workshops.
                </p>
              </div>
            </div>
          )}
          ...
          {wizardStep === 6 && (
            <div className="space-y-4 font-mono text-xs">
              ...
              <div className="p-4 bg-[#F7F5F0] border border-[#E5E1D8] rounded-md space-y-3">
                ...
                >
                  Notice Period Readiness: {noticeStatus} ({prepDays} days buffer satisfies AICTE & institutional lead time standards)
                </div>
              </div>
            </div>
          )}
              ...
              <div className="lg:col-span-2 bg-[#FFFFFF] rounded-md border border-[#E5E1D8] p-6 space-y-6 shadow-2xs font-mono text-xs">
                ...
                <div className="p-5 bg-[#FAF7F2] border border-[#B08D57]/40 rounded-md space-y-4">
                  ...
                  <div className="space-y-3 pt-2">
                    <div>
                      <div className="flex justify-between text-xs mb-1">
                        ...
                        <span className="font-bold text-[#111827]">41%</span>
                      </div>
                      <div className="h-3 bg-[#E5E1D8] rounded-full overflow-hidden">
                        <div className="h-full bg-stone-500 rounded-full" style={{ width: "41%" }} />
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="flex justify-between text-xs mb-1">
                        ...
                        <span className="font-bold text-[#4F6F5A]">72% (+31% Verified Gain)</span>
                      </div>
                      <div className="h-3 bg-[#E5E1D8] rounded-full overflow-hidden">
                        <div className="h-full bg-[#4F6F5A] rounded-full" style={{ width: "72%" }} />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="text-[11px] text-[#475569] pt-2 border-t border-[#E5E1D8]/60 flex items-center gap-1.5">
                    ...
                    <span>
                      148 Evidence records automatically published to Student Skill Passports with cryptographically verifiable checksums.
                    </span>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-[#FFFFFF] border border-[#E5E1D8] rounded-md max-w-lg w-full p-6 space-y-4 font-mono text-xs">
            <div className="flex items-start justify-between border-b border-[#E5E1D8] pb-3">
              <div>
                <span className="text-[10px] text-[#B08D57] uppercase tracking-wider">Closed-Loop Evidence Engine</span>
                <h3 className="text-lg font-normal text-[#111827] mt-1 font-sans" style={{ fontFamily: "var(--font-display)" }}>
                  Record Pre/Post Outcomes & Issue Passports
                </h3>
              </div>
            </div>
            ...
            <form onSubmit={handleRecordOutcomesSubmit} className="space-y-3">
              ...
              <div className="p-3 bg-[#FAF7F2] border border-[#B08D57]/30 rounded-sm">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={outcomeForm.generate_passports}
                    onChange={(e) => setOutcomeForm({ ...outcomeForm, generate_passports: e.target.checked })}
                  />
                  <span>Publish verified evidence badges to enrolled students' Passports</span>
                </label>
              </div>
              ...
              <div className="pt-3 border-t border-[#E5E1D8] flex items-center justify-end gap-2">
                ...
                >
                  <CheckCircle2 className="h-3.5 w-3.5" />
                  <span>{savingOutcomes ? "Recording..." : "Save & Commit Evidence"}</span>
                </button>
              </div>
            </form>
          </div>
