...
export const api = {
  ...
  getGitHubLoginUrl: (role: "student" | "recruiter" = "student") => request<GitHubOAuthURLResponse>(`/auth/github/login?role=${encodeURIComponent(role)}`),
  exchangeGitHubCode: (input: GitHubAuthExchangeRequest) => request<AuthSession>("/auth/github/exchange", { method: "POST", body: JSON.stringify(input) }),
